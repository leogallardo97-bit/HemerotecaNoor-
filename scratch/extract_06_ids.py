...
DB_COPY = os.path.join(os.path.dirname(__file__), 'drive_meta_copy_06.db')
shutil.copy2(DB_SRC, DB_COPY)
conn = sqlite3.connect(DB_COPY)
c = conn.cursor()

# Buscar la carpeta 06_PRODUCTO_CONTEXTUALIZACION
c.execute("""
    SELECT stable_id, id, local_title 
    FROM items 
    WHERE local_title = '06_PRODUCTO_CONTEXTUALIZACION' AND is_folder = 1
""")
folder = c.fetchone()

if not folder:
    print("Carpeta 06_PRODUCTO_CONTEXTUALIZACION no encontrada en Drive FS")
    sys.exit(1)
...
out_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), '06_ids.json')
with open(out_path, 'w', encoding='utf-8') as f:
    json.dump(results, f, ensure_ascii=False, indent=2)

# Also save folder info
folder_info_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), '06_folder_info.json')
with open(folder_info_path, 'w', encoding='utf-8') as f:
    json.dump({'cloud_id': FOLDER_CLOUD_ID}, f)
