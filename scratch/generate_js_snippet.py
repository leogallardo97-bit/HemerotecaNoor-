...
if match:
    ...
    output_path = r'c:\Users\leoga\Desktop\Noor\Antigravity Macro- Proyecto Drive\scratch\js_snippet_books.js'
    with open(output_path, 'w', encoding='utf-8') as out:
        out.write("    // --- Enlace Masivo 02_LIBROS (v1.4) ---\n")
        for doc in noor_docs:
            json_str = json.dumps(doc, indent=6, ensure_ascii=False)
            # Indent slightly better for pasting
            indented = json_str.replace('\n', '\n    ')
            out.write(f"    {indented},\n")
    
    print(f"JS snippet generated with {len(noor_docs)} books.")
else:
